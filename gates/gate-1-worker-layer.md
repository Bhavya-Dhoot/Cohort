...
**Addendum — founder free-tier policy (same day):** `models.yaml` now defaults to the `auto:free` sentinel. At spawn, the platform queries the live catalog (`GET /provider`), filters zero-cost tool-call-capable models on connected providers, and picks the newest (currently `opencode/hy3-free`, 2026-06-26, one of six free Zen models here). Resolution re-runs on every MCP server start, so catalog updates are tracked automatically; explicit `provider/model` strings still override. **Smoke re-run under `auto:free`: PASSED at $0.00 total cost.** Worker LLM spend is now zero-marginal-cost by default.

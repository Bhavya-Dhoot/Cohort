...
describe("renderAgentFile", () => {
  ...
  it("drops a spec key that a denyFloor glob covers by prefix, even without a matching trailing *", () => {
    // "git push" (no "*") overlaps the floor's "git push*" deny, but isn't
    // textually identical to it -- the merge must not let it survive as a
    // separate "allow" rule alongside the floor's "deny".
    const spec = makeSpec({
      permission: {
        "git push": "allow",
        "read *": "allow"
    ...
    // The spec's own "git push" key must not survive with "allow".
    expect(permission["git push"]).toBeUndefined();
    // The floor's deny for the covering glob is present.
    expect(permission["git push*"]).toBe("deny");
    // A non-overlapping spec key still passes through untouched.
    expect(permission["read *"]).toBe("allow");
  });

  it("stamps the generatedBy provenance marker in frontmatter", () => {
    const content = renderAgentFile(makeSpec(), []);
    const fm = parseFrontmatter(content);
    expect(fm.generatedBy).toBe("cohort");
  });
});
...
describe("generateSpecialist / retireSpecialist / listSpecialists", () => {
  ...
  beforeEach(() => {
    projectDir = join(tmpdir(), `cohort-specialist-test-${randomBytes(6).toString("hex")}`);
  });
  ...
  it("listSpecialists parses non-generated .md files best-effort (no role, no throw)", async () => {
    ...
    expect(byId.notes.generatedByUs).toBe(false);
    expect(byId["oauth-engineer"].generatedByUs).toBe(true);
  });

  it("retireSpecialist removes a file this module generated (carries the generatedBy marker)", async () => {
    const spec = makeSpec();
    await generateSpecialist({ projectDir, spec, denyFloor: DENY_FLOOR });
    ...
    await expect(stat(join(projectDir, ".opencode", "agent", "oauth-engineer.md"))).rejects.toThrow();
  });

  it("does not remove a hand-authored agent file lacking the generatedBy marker", async () => {
    const { mkdir, writeFile } = await import("node:fs/promises");
    const dir = join(projectDir, ".opencode", "agent");
    await mkdir(dir, { recursive: true });
    const filePath = join(dir, "hand-written.md");
    const content = "---\ndescription: hand-authored, not ours\nmode: subagent\n---\n\nDo not touch me.\n";
    await writeFile(filePath, content, "utf8");

    const result = await retireSpecialist({ projectDir, agentId: "hand-written" });
    expect(result).toEqual({ removed: false, reason: "not generated by cohort" });

    // File must still be there, untouched.
    const stillThere = await readFile(filePath, "utf8");
    expect(stillThere).toBe(content);
  });
